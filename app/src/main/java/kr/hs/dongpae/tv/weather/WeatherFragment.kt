        ...
        weather?.call("getWeather")?.let {
            ...
            withContext(Dispatchers.Main) {
                ...
                Glide.with(this@WeatherFragment.requireContext()).load(when (data.getString("weather")) {
                    ...
                    else -> R.drawable.wallpaper_cloud
...
//                Glide.with(this@WeatherFragment.requireContext()).load(R.drawable.wallpaper_lightning).into(image_weather_background)
                ...
                text_weather_temperature.text = data.getInt("temperature").toString() + context?.getString(R.string.weather_temp)
                ...
                text_weather_pm2.text = data.getInt("pm2").toString()
                view_weather_pm2.backgroundTintList = when (data.getInt("pm2")) {
                    in 0..15 -> activity!!.getColorStateList(R.color.accentedBlue)
                    in 16..35 -> activity!!.getColorStateList(R.color.accentedGreen)
                    in 36..75 -> activity!!.getColorStateList(R.color.accentedOrange)
                    else -> activity!!.getColorStateList(R.color.accentedRed)
                }
                ...
                JSONArray(data.getString("hourly")).listObject().forEachIndexed { index, item ->
                    ...
                    viewList[index].text_weather_item_time.text = "${item.getInt("time")}${context?.getString(R.string.weather_hour)}"
                    viewList[index].text_weather_item_temperature.text = "${item.getInt("temperature")}${context?.getString(R.string.weather_temp)}"
                }
